...
function Listing({ listing, handleQuickView }) {
  const image = listing?.asset?.photos?.[0]?.url || "https://placehold.co/600x400?text=No+Image";
      ...
      <div className="flex flex-col flex-1 p-4">
        ...
        <div className="mt-3">
          <div className="flex justify-between items-center mb-1 text-xs">
            <span className="font-semibold">{formatETB(raised)}</span>
        ...
        <div className="gap-2 grid grid-cols-3 mt-3 text-center">
          <div className="bg-base-200 py-1.5 rounded-lg">
            ...
            <p className="font-bold text-primary text-xs">{roi}%</p>
          </div>
          <div className="bg-base-200 py-1.5 rounded-lg">
            <p className="flex justify-center items-center gap-1 text-[10px] text-muted-foreground">
              <Users className="w-2.5 h-2.5" />
            </p>
            <p className="font-bold text-xs">{investors}</p>
          </div>
          <div className="bg-base-200 py-1.5 rounded-lg">
            <p className="flex justify-center items-center gap-1 text-[10px] text-muted-foreground">
              <Clock className="w-2.5 h-2.5" />
            </p>
            <p className="font-bold text-xs">{daysRemaining}d</p>
          </div>
        </div>
        <div className="flex gap-2 mt-3 pt-3 border-base-200 border-t">
          ...
          <Link
            to={`/investor/marketplace/${listing._id}`}
